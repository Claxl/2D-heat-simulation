...
//         It currently only gives the value of a. DONE
...
{
    // TODO 3: Implement the swap function to swap the content of two variables. DONE
    real_t* temp;
    temp =*m1;
    *m1 = *m2;
    *m2 = temp;
...
main ( int argc, char** argv )
{
    ...
    //         and store the values in the global fields with the same names. DONE
    ...
    //         using the structs t_start and t_end declared above.  DONE
...
domain_init ( void )
{
    ...
    //         with enough space for N real_t elements.
    for(int i = 0; i<3; i++){
        temp[i] = (real_t*)malloc(sizeof(real_t)*N);
    }
