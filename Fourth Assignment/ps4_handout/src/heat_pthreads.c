...
void * threadFunction(void* arg){
    int_t row = N/n_threads;
    int_t start = (int_t)arg*row;
    int_t new_N = start+row;
    if((int_t)arg == n_threads-1){
        new_N += N%n_threads    ;
    }
    time_step(start, new_N);
}

void
time_step ( int_t start, int_t end)
{
    real_t c, t, b, l, r, K, new_value;

    for ( int_t y = 1; y <= M; y++ )
    {
        for ( int_t start = 1; start <= end; start++ )
        {
            c = T(start, y);
            ...
            l = T(start, y - 1);
            r = T(start, y + 1);
            K = THERMAL_DIFFUSIVITY(start, y);

            new_value = c + K * dt * ((l - 2 * c + r) + (b - 2 * c + t));

            T_next(start, y) = new_value;
        }
    }
}
